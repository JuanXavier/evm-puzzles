...
function printCode(code) {
	...
	while (i < code.length) {
		...
		opcodeHexColumnWidth = Math.max(opcodeHexColumnWidth, opcodeHexItem.value.length);
...
function getNextPuzzle() {
	...
	const numberOfPuzzles = fs.readdirSync(resolve(root, 'puzzles')).filter((x) => !x.startsWith('.')).length;
...
function saveSolution(puzzleNumber, solution) {
	...
	fs.writeJsonSync(resolve(solutionsDir, `solution_${puzzleNumber}.json`), solution);
}
